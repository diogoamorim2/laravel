...
## 2024-05-30 - Cast Input Strings to Prevent Array Payload DoS
**Vulnerability:** Similar to previous type juggling bugs, `RateLimiter::tooManyAttempts($emailKey, 3)` relies on string keys built using `Str::lower($request->input('email'))`. Passing an array like `email[]=value` crashed the application with a `TypeError` due to `Str::lower()` expecting a string, allowing a Denial of Service.
**Learning:** We must not only validate inputs using FormRequests but also explicitly cast request data to the expected scalar types (`(string)`, `(int)`) *before* using them in string manipulation or math operations. FormRequest validation runs *before* Controller logic, but in this specific route structure, the rate limit check runs manually inside the controller before validation happens or in case validation rules are bypassed.
**Prevention:** As a defensive programming measure, explicitly cast request inputs to strings (e.g., `(string) $request->input('email')`) before passing them to string functions or logs in controllers. For pagination parameters used in math (e.g., `$request->input('page')`), cast them to `(int)`. This adds defense-in-depth against PHP 8+ TypeError DoS vulnerabilities.
