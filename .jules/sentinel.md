...
## 2024-05-30 - Array Payload DoS in Rate Limiting & Pagination
**Vulnerability:** The application was vulnerable to a Denial of Service (DoS) attack via array payloads in HTTP requests. In `ContatoController@store`, the `email` input was passed directly to `Str::lower()` for rate limiting. In `ContatoController@index`, the `page` input was used in mathematical operations. If an attacker provided an array (e.g., `?page[]=1` or `email[]=test`), PHP 8+ threw a `TypeError`, resulting in a 500 Internal Server Error.
**Learning:** Functions expecting strings or integers (like `Str::lower` or math operations) will crash if given an array from unvalidated user input. This is a common attack vector to cause application instability. FormRequest validation runs *before* the controller, but in the case of `page` (which wasn't validated) and `email` (where rate limiting happened *before* validation), the raw input was used directly.
**Prevention:** Always cast or validate input types before using them in functions that expect specific scalar types, especially when the input hasn't passed FormRequest validation yet or isn't covered by it (like pagination parameters). For rate limiting keys, ensure the input is a string. For pagination math, explicitly cast to `(int)`.
