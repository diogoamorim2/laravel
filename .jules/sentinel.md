...
## 2024-05-30 - Array Input Denial of Service (DoS) in Arithmetic Operations
**Vulnerability:** The application crashed with a 500 TypeError when an array payload (e.g., `?page[]=1`) was submitted to the `page` query parameter used in mathematical operations for pagination math `(request()->input('page', 1) - 1) * 5`.
**Learning:** In PHP 8+, passing array payloads from unvalidated requests to arithmetic operations triggers a fatal `TypeError` (Unsupported operand types: array - int), which can lead to a Denial of Service.
**Prevention:** HTTP query parameters used in mathematical operations must be explicitly cast to integers (e.g., `(int) request()->input('page')`) to safely handle array payloads before performing math.
