...
## 2026-04-10 - Array Payload Denial of Service (Pagination)
**Vulnerability:** A  exception was thrown when the application performed arithmetic operations (e.g., ) on HTTP input variables. By passing an array payload (e.g., ), attackers could trigger an unhandled exception and crash the worker, potentially leading to a Denial of Service condition in PHP 8+.
**Learning:** In PHP 8+, math operations are stricter. The native Laravel  safely handles array inputs internally, but custom arithmetic calculations directly calling  do not. This reveals a specific vulnerability pattern where manual math operations on user input can be exploited via arrays.
**Prevention:** Always explicitly typecast values retrieved from  (e.g., using ) before performing mathematical operations, particularly if they are not validated by a FormRequest first.
## 2026-03-06 - Array Payload Denial of Service (Pagination)
**Vulnerability:** A `TypeError` exception was thrown when the application performed arithmetic operations (e.g., `request()->input('page') - 1`) on HTTP input variables. By passing an array payload (e.g., `?page[]=1`), attackers could trigger an unhandled exception and crash the worker, potentially leading to a Denial of Service condition in PHP 8+.
**Learning:** In PHP 8+, math operations are stricter. The native Laravel `paginate()` safely handles array inputs internally, but custom arithmetic calculations directly calling `request()->input()` do not. This reveals a specific vulnerability pattern where manual math operations on user input can be exploited via arrays.
**Prevention:** Always explicitly typecast values retrieved from `request()->input()` (e.g., using `(int)`) before performing mathematical operations, particularly if they are not validated by a FormRequest first.
