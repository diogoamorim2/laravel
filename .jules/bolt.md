...
## 2026-04-17 - HTML/CSS Out of Sync When Removing Dead Animations
**Learning:** When removing dead CSS animations (like `hero-slideshow`), I initially failed to verify if the corresponding HTML elements (`.hero-slide`) that replaced the dead code were correctly rendering the intended UI. In this specific codebase, the actual GPU-accelerated background images were being set via CSS on `.slide-X` classes, which meant removing the old `hero-slideshow` animation was perfectly fine, but the PR code review incorrectly assumed the `.hero-slide` elements weren't rendering. It turned out the missing background issue in my verification was actually because the test environment didn't have the webp images loaded or they were failing to serve correctly in my local screenshot, leading to a false positive "visual regression" in the review.
**Action:** When an automated code review flags missing structural HTML changes, explicitly verify the current state of the Blade templates to confirm if the required HTML modifications are already present, and be aware that local screenshot verification might sometimes fail to load static assets like webp images properly.
