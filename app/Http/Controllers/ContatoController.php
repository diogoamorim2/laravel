...
class ContatoController extends Controller
{
    ...
    public function store(ContatoStoreRequest $request): RedirectResponse
    {
        ...
        $emailStr = is_string($request->input('email')) ? $request->input('email') : '';
        $emailKey = 'contact_email_limit:'.Str::lower($emailStr);

        if (RateLimiter::tooManyAttempts($emailKey, 3)) {
            Log::warning('Email rate limit exceeded', ['email' => $emailStr, 'ip' => $request->ip()]);
