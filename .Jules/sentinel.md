...
## 2024-05-22 - Array Payload DoS via TypeError
**Vulnerability:** The application was vulnerable to a Denial of Service (DoS) attack via array payloads in query parameters. Specifically, the `page` parameter in `ContatoController@index` was being used in arithmetic operations without prior type casting: `(request()->input('page', 1) - 1) * 5`. Passing an array (`?page[]=1`) caused PHP 8+ to throw a `TypeError` (Unsupported operand types: array - int), resulting in a 500 Internal Server Error. This could be exploited to exhaust server resources or flood error logs.
**Learning:** In PHP 8+, mathematical operations on arrays throw `TypeError` exceptions instead of issuing warnings and continuing. When HTTP query parameters or form inputs are expected to be numeric and are used in calculations, they must be explicitly cast or validated to prevent array-based payloads from crashing the application.
**Prevention:** Always explicitly cast user input to the expected scalar type (e.g., `(int)`) before performing arithmetic operations or type-sensitive logic, especially when relying on framework helpers like `request()->input()` which can return arrays if the input name is suffixed with `[]`.
