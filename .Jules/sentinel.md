...
## 2024-04-11 - [CRITICAL] Prevent Array Payload DoS from Math and String Operations
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in `ContatoController` due to missing input type casting. Unvalidated request parameters like `page` and `email` were passed directly into mathematical operations or PHP 8 string functions. Submitting an array payload (e.g., `?page[]=1`) caused a `TypeError` which resulted in a 500 Internal Server Error, making the application susceptible to application-level DoS attacks.
**Learning:** In PHP 8+, string functions (like `Str::lower()`) and mathematical operators strictly reject arrays. Even seemingly harmless parameters like a `page` number or an `email` string can crash the application if an attacker manipulates the request to send an array payload instead of a scalar value. Form requests and validators only protect if they reject array types.
**Prevention:** Always explicitly type cast inputs derived from HTTP requests, even when they appear trivial. Use `(int)` for variables destined for arithmetic operations and `(string)` for variables destined for string manipulation functions. E.g., `(int) request()->input('page')` and `(string) request()->input('email')`.
