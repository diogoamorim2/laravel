...
## 2026-11-06 - TypeError Array Payload DoS
**Vulnerability:** The `ContatoController@index` method used mathematical operations on the `page` query parameter (e.g., `request()->input('page', 1) - 1`) to calculate an index variable. This was vulnerable to a Denial of Service via Array Payload DoS, as passing an array (`?page[]=1`) caused a `TypeError` and a 500 error when the array was used in subtraction.
**Learning:** In PHP 8+, mathematical or string operations that expect scalars will throw a fatal `TypeError` if provided an array. Since query parameters can easily be converted to arrays by attackers, directly using them without type checking or casting in arithmetic operations is an exploitable DoS vulnerability.
**Prevention:** Always explicitly cast user input to the expected scalar type (e.g., `(int) request()->input('page', 1)`) before using it in mathematical operations or passing it to native functions expecting scalars, especially when not covered by standard FormRequest validation.
